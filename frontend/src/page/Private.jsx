  ...
  const fetchPosts = useCallback(async (pageNum = 0) => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/posts/private`,
        {
          params: {
            username,
            page: pageNum,
            ...
            <div className='A'>
              {posts.map((post) => (
                <div className="post_item" key={post.id}>
                  <div
                    className='private_post'
                    onClick={() => navigate(`/private/posts/${post.id}`)}
                  >
                    <p className='title'>
                    ...
                    {post.thumbnail ? (
                      ...
                      <div
                        className="post_thumbnail empty"
                        style={{
